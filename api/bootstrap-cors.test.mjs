...
function makePreflight(origin) {
  return new Request('https://api.wm.vercel.app/api/bootstrap?keys=techReadiness', {
    method: 'OPTIONS',
    headers: {
      origin,
...
test('bootstrap preflight is compatible with credentialed browser fetches', async () => {
  const resp = await handler(makePreflight('https://wm.vercel.app'));

  assert.equal(resp.status, 204);
  assert.equal(resp.headers.get('access-control-allow-origin'), 'https://wm.vercel.app');
...
test('bootstrap GET response is compatible with credentialed browser fetches', async () => {
  ...
  try {
    ...
    const resp = await handler(new Request('https://api.wm.vercel.app/api/bootstrap?keys=techReadiness', {
      method: 'GET',
      headers: {

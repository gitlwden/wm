...
function makePreflight(origin) {
  return new Request('https://wm-worldmonitor.netlify.app/api/health?compact=1', {
    method: 'OPTIONS',
    headers: {
      origin,
...
test('health GET response is compatible with credentialed browser fetches', async () => {
  const resp = await handler(new Request('https://wm-worldmonitor.netlify.app/api/health?compact=1', {
    method: 'GET',
    headers: {

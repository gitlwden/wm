...
function makeReportRequest({ body, contentType = 'application/reports+json' } = {}) {
  return new Request('https://wm-worldmonitor.netlify.app/api/security/report', {
    method: 'POST',
    ...
    body: JSON.stringify([
      {
        type: 'coep',
        age: 10,
        url: 'https://tech.wm-worldmonitor.netlify.app/panel?token=secret',
        body: {
...
test('security report endpoint rejects unsupported methods', async () => {
  const response = await handler(new Request('https://wm-worldmonitor.netlify.app/api/security/report'));

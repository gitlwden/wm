...
<body><a href="${_BASE}" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">WorldMonitor MCP</span></a>
<div class="card"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(detail)}</p><a href="javascript:history.back()" class="back">&#8592; go back</a></div>
<p class="footer"><a href="${_BASE}" target="_blank" rel="noopener">${_HOST}</a></p>
...
export function consentPage(params, nonce, errorMsg = '') {
    ...
    const proCtaHref = `${_BASE}/mcp-grant?nonce=${encodeURIComponent(nonce)}`;
...
<body>
<a href="${_BASE}" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">WorldMonitor MCP</span></a>
<div class="card">
<div class="client-hd">
...
<form id="cf" method="POST" action="${_BASE}/oauth/authorize" style="display:none;margin-top:1.25rem">
...
<p class="hint">No key? <a href="${_BASE}/pro" target="_blank" rel="noopener">Get one at ${_HOST}/pro &#x2192;</a></p>
<p class="error" id="ke"${errorMsg ? '' : ' style="display:none"'}>${errorMsg ? escapeHtml(errorMsg) : ''}</p>
<button type="submit" id="ab">Authorize</button>
</form>
</div>
<p class="footer"><a href="${_BASE}" target="_blank" rel="noopener">${_HOST}</a> &middot; <a href="${_BASE}/pro" target="_blank" rel="noopener">Get an API key &#x2192;</a></p>
...
export default async function handler(req) {
    ...
    if (method === 'GET') {
        ...
        const nonceStored = await redisSet(`oauth:nonce:${nonce}`, {
            client_id,
            redirect_uri,
            code_challenge,
            state,
    ...
    if (method === 'POST') {
        ...
        // if (origin && origin !== '${_BASE}' && origin !== 'null') {

...
export const ensureRecord = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.localeTag.length > MAX_LOCALE_TAG_LEN ||
            !LOCALE_TAG_RE.test(args.localeTag)) {
            console.warn(`[users:ensureRecord] invalid localeTag rejected: ${args.localeTag.slice(0, 64)}`);
            return { ok: false, reason: "invalid-input", field: "localeTag" };
        }
        if (args.localePrimary.length > MAX_LOCALE_PRIMARY_LEN ||
            !LOCALE_PRIMARY_RE.test(args.localePrimary)) {
            console.warn(`[users:ensureRecord] invalid localePrimary rejected: ${args.localePrimary.slice(0, 64)}`);
            return { ok: false, reason: "invalid-input", field: "localePrimary" };
        }
        if (args.timezone !== undefined) {
            if (args.timezone.length > MAX_TIMEZONE_LEN || !isValidTimezone(args.timezone)) {
                console.warn(`[users:ensureRecord] invalid timezone rejected: ${args.timezone.slice(0, 64)}`);
                return { ok: false, reason: "invalid-input", field: "timezone" };
            }
        }
        if (args.country !== undefined && !COUNTRY_RE.test(args.country)) {
            console.warn(`[users:ensureRecord] invalid country rejected: ${args.country.slice(0, 64)}`);
            return { ok: false, reason: "invalid-input", field: "country" };
        }
        // ──── Auth ────
        ...
        await ctx.db.insert("users", {
            ...
            normalizedEmail: incomingNormalizedEmail.length > 0 ? incomingNormalizedEmail : undefined,
            localeTag: args.localeTag,
            localePrimary: args.localePrimary,

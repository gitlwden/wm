...
function featureCardsHtml(planKey) {
...
function userWelcomeHtml(planName, planKey) {
...
function buildPriceRowsHtml(args) {
    ...
    if (typeof paid === "number") {
        ...
        rows.push(`<tr><td style="color: #888; padding-right: 16px;">Amount Paid:</td><td style="color: #fff;">${formatMoney(paid, currency)}${taxNote}</td></tr>`);
        ...
        if (currency.toUpperCase() === "USD" &&
            ...
            rows.push(`<tr><td style="color: #888; padding-right: 16px;">List Price:</td><td style="color: #fff;">${formatMoney(listCents, currency)}</td></tr>`);
            if (savedCents > 0) {
                rows.push(`<tr><td style="color: #888; padding-right: 16px;">Saved:</td><td style="color: #4ade80;">${formatMoney(savedCents, currency)} (${pct}% off)</td></tr>`);
            }
        }
    }
    if (args.discountId) {
        rows.push(`<tr><td style="color: #888; padding-right: 16px;">Discount:</td><td style="color: #fff; font-size: 12px;">${args.discountId}</td></tr>`);
    }
    return rows.join("");
}
/**
...
export const sendSubscriptionEmails = internalAction({
    ...
    handler: async (_ctx, args) => {
        ...
        await sendEmail(apiKey, args.userEmail, `Welcome to World Monitor ${planName}`, userWelcomeHtml(planName, args.planKey), ADMIN_EMAIL);
        ...
        await sendEmail(apiKey, ADMIN_EMAIL, `[WM] New User Subscribed to ${planName}`, `<div style="font-family: monospace; padding: 20px; background: #0a0a0a; color: #e0e0e0;">

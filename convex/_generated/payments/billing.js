 ...
 */
async function createCustomerPortalUrlForUser(ctx, userId) {
    const dodoCustomerId = await ctx.runQuery(internal.payments.billing.getDodoCustomerIdForUserPortal, { userId });
    if (!dodoCustomerId) {
...
function getSubscriptionStatusPriority(status) {
    switch (status) {
        case "active":
            return 0;
        case "on_hold":
            ...
            rows.push({
                ...
                subscriptions: subs.map((s) => {
                    ...
                    return {
                        ...
                        rawPayloadCustomerId: typeof p?.customer?.customer_id === "string"
                            ? p.customer.customer_id
                            : null,
                        rawPayloadEmail: typeof p?.customer?.email === "string" ? p.customer.email : null,
                    };
                }),
            });
        }
...
export const repairCustomerFromSubscriptionPayload = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const priority = (status) => status === "active" ? 0 :
            status === "on_hold" ? 1 :
                status === "cancelled" ? 2 :
                    3;
        subs.sort((a, b) => {
        ...
        for (const sub of subs) {
            ...
            if (collidingByDodo && collidingByDodo.userId !== args.userId) {
                console.warn(`[billing/repair] customers.dodoCustomerId=${dodoCustomerId} already mapped to userId=${collidingByDodo.userId}; refusing to remap to userId=${args.userId}.`);
                return null;
            }
            ...
            const existingByUser = await ctx.db
                ...
                .first();
            if (existingByUser) {
                console.warn(`[billing/repair] Patching dodoCustomerId=${dodoCustomerId} into existing customers row for userId=${args.userId} (dodoSubscriptionId=${sub.dodoSubscriptionId}). Webhook gap — investigate subscriptionHelpers.ts:520-549.`);
            ...
            console.warn(`[billing/repair] Inserting customers row for userId=${args.userId} from subscription rawPayload (dodoSubscriptionId=${sub.dodoSubscriptionId}). Webhook gap — investigate subscriptionHelpers.ts:520-549.`);
            ...
            for (const sub of subs) {
                ...
                if (collision) {
                    if (collision.userId !== userId) {
                        ...
                        console.warn(`[billing/backfill] cross-user collision: dodoCustomerId=${dodoCustomerId} already maps to userId=${collision.userId}; skipping userId=${userId}.`);
                        break;
                    }
                ...
                if (existing) {
                    console.warn(`[billing/backfill] Patching dodoCustomerId=${dodoCustomerId} into existing customers row for userId=${userId} (dodoSubscriptionId=${sub.dodoSubscriptionId}).`);
                    ...
                    console.warn(`[billing/backfill] Inserted customers row for userId=${userId} from subscription dodoSubscriptionId=${sub.dodoSubscriptionId}.`);
                }
                repairedThisUser = true;
                break;
            }
    ...
    handler: async (ctx, args) => {
        ...
        const blockingSubs = (await ctx.db
            ...
            if (!existingCatalogEntry)
                return false;
            if (existingCatalogEntry.tierGroup !== targetCatalogEntry.tierGroup)
                return false;
...
export const claimSubscription = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        let winningPlanKey = null;
        let winningFeatures = null;
        let winningValidUntil = null;
        if (anonEntitlement) {
            ...
            if (existingEntitlement) {
                ...
                const anonWins = anonTier > existingTier ||
        ...
        if (process.env.UPSTASH_REDIS_REST_URL) {
            ...
            await ctx.scheduler.runAfter(0, internal.payments.cacheActions.deleteEntitlementCache, { userId: args.anonId });
...
export const grantComplimentaryEntitlement = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!PRODUCT_CATALOG[args.planKey]) {
            throw new Error(`grantComplimentaryEntitlement: unknown planKey "${args.planKey}". Must be in PRODUCT_CATALOG.`);
        }
        const now = Date.now();
        ...
        console.log(`[billing] grantComplimentaryEntitlement userId=${args.userId} planKey=${args.planKey} days=${args.days} validUntil=${new Date(validUntil).toISOString()}${args.reason ? ` reason="${args.reason}"` : ""}`);
        // Sync Redis cache so edge gateway sees the comp without waiting for TTL.
        if (process.env.UPSTASH_REDIS_REST_URL) {
            await ctx.scheduler.runAfter(0, internal.payments.cacheActions.syncEntitlementCache, { userId: args.userId, planKey: args.planKey, features, validUntil });
        }
        return {
            userId: args.userId,
            planKey: args.planKey,
...
export const deleteSubscriptionByDodoId = internalMutation({
    ...
    handler: async (ctx, args) => {
        const sub = await ctx.db
            .query("subscriptions")
            .withIndex("by_dodoSubscriptionId", (q) => q.eq("dodoSubscriptionId", args.dodoSubscriptionId))
            .unique();
        if (!sub) {
            throw new Error(`[billing] deleteSubscriptionByDodoId: no subscription found with dodoSubscriptionId="${args.dodoSubscriptionId}"`);
        }
        const userId = sub.userId;
        await ctx.db.delete(sub._id);
        console.log(`[billing] deleteSubscriptionByDodoId userId=${userId} dodoSubscriptionId=${args.dodoSubscriptionId} planKey=${sub.planKey} reason="${args.reason}"`);

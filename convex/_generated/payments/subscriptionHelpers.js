 ...
 */
export function isNewerEvent(existingUpdatedAt, incomingTimestamp) {
 ...
 */
export async function upsertEntitlements(ctx, userId, planKey, validUntil, updatedAt) {
    ...
    if (process.env.UPSTASH_REDIS_REST_URL) {
        await ctx.scheduler.runAfter(0, internal.payments.cacheActions.syncEntitlementCache, { userId, planKey, features, validUntil });
    }
}
/**
 ...
 */
export function compareSubscriptionsByCoverage(a, b) {
 ...
 * correctly excluded.
 */
async function pickBestCoveringSub(ctx, userId, at) {
 ...
 */
export async function recomputeEntitlementFromAllSubs(ctx, userId, eventTimestamp) {
    const entitlement = await ctx.db
        .query("entitlements")
        .withIndex("by_userId", (q) => q.eq("userId", userId))
        .first();
    if (entitlement?.compUntil && entitlement.compUntil > eventTimestamp) {
        console.log(`[subscriptionHelpers] recompute for ${userId} — comp floor active until ${new Date(entitlement.compUntil).toISOString()}, preserving entitlement`);
        return;
    }
 ...
 */
async function resolvePlanKey(ctx, dodoProductId) {
    ...
    if (aliasedPlan) {
        console.warn(`[subscriptionHelpers] Resolved "${dodoProductId}" via legacy alias → "${aliasedPlan}". ` +
    ...
    console.error(`[subscriptionHelpers] Unknown Dodo product ID "${dodoProductId}" — ` +
 ...
 */
async function resolveUserId(ctx, dodoCustomerId, metadata) {
    ...
    if (metadata?.wm_user_id && metadata?.wm_user_id_sig) {
        ...
        console.warn(`[subscriptionHelpers] Invalid HMAC signature for wm_user_id="${metadata.wm_user_id}" — ignoring metadata`);
    }
    else if (metadata?.wm_user_id && !metadata?.wm_user_id_sig) {
        console.warn(`[subscriptionHelpers] Unsigned wm_user_id="${metadata.wm_user_id}" — ignoring (requires HMAC signature)`);
    }
    ...
    // 3. Dev-only fallback
    if (isDev) {
        console.warn(`[subscriptionHelpers] No user identity found for customer="${dodoCustomerId}" — using dev fallback "${DEV_USER_ID}"`);
        return DEV_USER_ID;
    }
    throw new Error(`[subscriptionHelpers] Cannot resolve userId: no verified metadata, no customer record, no dodoCustomerId.`);
}
/**
 ...
 */
function toEpochMs(value, fieldName, fallback) {
    ...
    console.warn(`[subscriptionHelpers] toEpochMs: missing or invalid ${fieldName ?? "date"} value (${String(value)}) — falling back to ${fallback !== undefined ? "eventTimestamp" : "Date.now()"}`);
    return fb;
}
// ---------------------------------------------------------------------------
 ...
 */
export async function handleSubscriptionActive(ctx, data, eventTimestamp) {
    const planKey = await resolvePlanKey(ctx, data.product_id);
    const userId = await resolveUserId(ctx, data.customer?.customer_id ?? "", data.metadata);
            ...
            if (referrer) {
                ...
                if (refereeEmail) {
                    const existingCredit = await ctx.db
                        ...
                        .withIndex("by_referrer_email", (q) => q.eq("referrerUserId", referrer.userId).eq("refereeEmail", refereeEmail))
                        .first();
                    if (!existingCredit) {
    ...
    if (!email) {
        console.warn(`[subscriptionHelpers] subscription.active: no customer email — skipping welcome email (subscriptionId=${data.subscription_id})`);
    }
    else if (existing) {
        ...
        await ctx.scheduler.runAfter(0, internal.payments.subscriptionEmails.sendSubscriptionEmails, {
            userEmail: email,
            planKey,
            userId,
 ...
 */
export async function handleSubscriptionRenewed(ctx, data, eventTimestamp) {
    const existing = await ctx.db
        .query("subscriptions")
        .withIndex("by_dodoSubscriptionId", (q) => q.eq("dodoSubscriptionId", data.subscription_id))
        .unique();
    if (!existing) {
        console.warn(`[subscriptionHelpers] Renewal for unknown subscription ${data.subscription_id} -- skipping`);
        return;
    }
    if (!isNewerEvent(existing.updatedAt, eventTimestamp))
        return;
 ...
 */
export async function handleSubscriptionOnHold(ctx, data, eventTimestamp) {
    const existing = await ctx.db
        .query("subscriptions")
        .withIndex("by_dodoSubscriptionId", (q) => q.eq("dodoSubscriptionId", data.subscription_id))
        .unique();
    if (!existing) {
        console.warn(`[subscriptionHelpers] on_hold for unknown subscription ${data.subscription_id} -- skipping`);
        return;
    }
    if (!isNewerEvent(existing.updatedAt, eventTimestamp))
        return;
    ...
    console.warn(`[subscriptionHelpers] Subscription ${data.subscription_id} on hold -- payment failure`);
 ...
 * Entitlements remain valid until `currentPeriodEnd` (no immediate revocation).
 */
export async function handleSubscriptionCancelled(ctx, data, eventTimestamp) {
    const existing = await ctx.db
        .query("subscriptions")
        .withIndex("by_dodoSubscriptionId", (q) => q.eq("dodoSubscriptionId", data.subscription_id))
        .unique();
    if (!existing) {
        console.warn(`[subscriptionHelpers] Cancellation for unknown subscription ${data.subscription_id} -- skipping`);
        return;
    }
    if (!isNewerEvent(existing.updatedAt, eventTimestamp))
        return;
 ...
 */
export async function handleSubscriptionPlanChanged(ctx, data, eventTimestamp) {
    const existing = await ctx.db
        .query("subscriptions")
        .withIndex("by_dodoSubscriptionId", (q) => q.eq("dodoSubscriptionId", data.subscription_id))
        .unique();
    if (!existing) {
        console.warn(`[subscriptionHelpers] Plan change for unknown subscription ${data.subscription_id} -- skipping`);
        return;
    }
    if (!isNewerEvent(existing.updatedAt, eventTimestamp))
        return;
 ...
 */
export async function handleSubscriptionExpired(ctx, data, eventTimestamp) {
    const existing = await ctx.db
        .query("subscriptions")
        .withIndex("by_dodoSubscriptionId", (q) => q.eq("dodoSubscriptionId", data.subscription_id))
        .unique();
    if (!existing) {
        console.warn(`[subscriptionHelpers] Expiration for unknown subscription ${data.subscription_id} -- skipping`);
        return;
    }
    if (!isNewerEvent(existing.updatedAt, eventTimestamp))
        return;
 ...
 * handler is needed.
 */
export async function handleSubscriptionUpdated(ctx, data, eventTimestamp) {
    ...
    switch (status) {
        ...
        default: {
            console.error(`[handleSubscriptionUpdated] unhandled status="${status}" sub=${data.subscription_id}; ` +
 ...
 */
export async function handlePaymentOrRefundEvent(ctx, data, eventType, eventTimestamp) {
    const userId = await resolveUserId(ctx, data.customer?.customer_id ?? "", data.metadata);
    ...
    if (eventType === "refund.succeeded" && data.subscription_id) {
        ...
        if (decision.kind === "alert") {
            console.error(`[refund-alert] full refund without prior cancellation: ` +
            ...
            // case exists.
            console.warn(`[refund-alert] refund on active sub but cannot classify amount: ` +
                `subId=${data.subscription_id} userId=${decision.userId} ` +
 ...
 */
export async function handleDisputeEvent(ctx, data, eventType, eventTimestamp) {
    const userId = await resolveUserId(ctx, data.customer?.customer_id ?? "", data.metadata);
    ...
    if (eventType === "dispute.lost") {
        console.warn(`[subscriptionHelpers] Dispute LOST for user ${userId}, payment ${data.payment_id} — revoking entitlement`);
        ...
        if (existing) {
            ...
            if (process.env.UPSTASH_REDIS_REST_URL) {
                await ctx.scheduler.runAfter(0, internal.payments.cacheActions.syncEntitlementCache, {
                    userId,
                    planKey: "free",
                    features: freeFeatures,
                    validUntil: eventTimestamp,

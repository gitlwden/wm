...
function buildBlockedCheckoutPayload(subscription) {
    return {
...
function buildBlockedCheckoutResponse(subscription) {
    return {
        blocked: true,
        ...buildBlockedCheckoutPayload(subscription),
    };
}
async function getCheckoutBlockingSubscription(ctx, userId, productId) {
    const result = await ctx.runQuery(internal.payments.billing.getCheckoutBlockingSubscription, { userId, productId });
...
async function _createCheckoutSession(ctx, args, user) {
    ...
    try {
        ...
        console.error(`[checkout] createCheckout failed for user=${user.userId} product=${args.productId}: ${msg}`);
...
export const internalCreateCheckout = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        return _createCheckoutSession(ctx, {

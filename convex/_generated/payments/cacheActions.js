 ...
 */
function getEntitlementKey(userId) {
...
export const syncEntitlementCache = internalAction({
    ...
    handler: async (_ctx, args) => {
        ...
        if (!url || !token) {
            console.warn("[cacheActions] UPSTASH_REDIS_REST_URL or UPSTASH_REDIS_REST_TOKEN not set -- skipping cache sync");
            return;
        }
        ...
        try {
            const resp = await fetch(`${url}/set/${encodeURIComponent(key)}/${encodeURIComponent(value)}/EX/${ENTITLEMENT_CACHE_TTL_SECONDS}`, {
                method: "POST",
            ...
            if (!resp.ok) {
                ...
                throw new Error(`[cacheActions] Redis SET failed: HTTP ${resp.status} for user ${args.userId}`);
            }
        }
        catch (err) {
            console.warn("[cacheActions] Redis cache sync failed:", err instanceof Error ? err.message : String(err));
...
export const deleteEntitlementCache = internalAction({
    ...
    handler: async (_ctx, args) => {
        ...
        try {
            ...
            if (!resp.ok) {
                console.warn(`[cacheActions] Redis DEL failed: HTTP ${resp.status} for key ${key}`);
            }
        }
        catch (err) {
            ...
            console.warn("[cacheActions] Redis cache delete failed:", err instanceof Error ? err.message : String(err));
        }
        finally {
            clearTimeout(timeout);
        }

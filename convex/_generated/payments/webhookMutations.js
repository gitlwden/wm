...
import { handleSubscriptionActive, handleSubscriptionRenewed, handleSubscriptionOnHold, handleSubscriptionCancelled, handleSubscriptionPlanChanged, handleSubscriptionExpired, handleSubscriptionUpdated, handlePaymentOrRefundEvent, handleDisputeEvent, } from "./subscriptionHelpers";
/**
...
export const processWebhookEvent = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!data || typeof data !== 'object') {
            throw new Error(`[webhook] rawPayload.data is missing or not an object (eventType=${args.eventType}, webhookId=${args.webhookId})`);
        }
        ...
        if (subscriptionEvents.includes(args.eventType) && !data.subscription_id) {
            throw new Error(`[webhook] Missing subscription_id for subscription event (eventType=${args.eventType}, webhookId=${args.webhookId}, dataKeys=${Object.keys(data).join(",")})`);
        }
        switch (args.eventType) {
            ...
            default:
                ...
                if (typeof args.eventType === "string" && args.eventType.startsWith("subscription.")) {
                    console.error(`[webhook] Unhandled subscription.* event type: ${args.eventType} — needs a dedicated handler in subscriptionHelpers.ts`);
                }
                else {

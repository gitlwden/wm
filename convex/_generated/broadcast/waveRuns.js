...
import { createSegment, upsertContactToSegment, } from "./_resendContacts";
 ...
 */
async function pushWithBackoff(apiKey, email, segmentId) {
    let lastResult;
    for (let attempt = 0; attempt < RESEND_BACKOFF_MAX_RETRIES; attempt++) {
        ...
        // ever shorter.
        const base = RESEND_BACKOFF_MS[attempt] ??
...
export const _getUsersByEmailPage = internalQuery({
    ...
    handler: async (ctx, { emails }) => {
        ...
        if (validEmails.length === 0)
            return [];
        const rows = await Promise.all(validEmails.map((email) => ctx.db
            .query("users")
...
export const _dryRunNonEnglishExclusion = internalAction({
    ...
    handler: async (ctx, args) => {
        const sampleSize = typeof args.sampleSize === "number" && args.sampleSize > 0
        ...
        let eligibleTotal = 0;
        let excludedTotal = 0;
        const excludedByLocale = {};
        const sampleExcludedEmails = [];
        let cursor = null;
        while (true) {
            const page = await ctx.runQuery(internal.broadcast.waveRuns._getRegistrationsPage, { cursor, numItems: REGISTRATIONS_PAGE_SIZE });
            ...
            if (dedup.length > 0) {
                const userRows = await ctx.runQuery(internal.broadcast.waveRuns._getUsersByEmailPage, { emails: dedup });
            ...
            if (sampleExcludedEmails.length < sampleSize) {
                ...
                for (const row of page.page) {
                    ...
                    if (!e)
                        continue;
                    if (suppressedSet.has(e) || paidSet.has(e) || row.proLaunchWave)
                        continue;
...
export const _claimWaveRunLease = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const collides = await ctx.db
            ...
            .first();
        if (collides)
            return { ok: false, reason: "label-collides" };
        const now = Date.now();
...
export const _persistPickedBatch = internalMutation({
    ...
    handler: async (ctx, { runId, contacts }) => {
        if (contacts.length > PERSIST_CHUNK_SIZE) {
            throw new Error(`[_persistPickedBatch] chunk too large: ${contacts.length} > ${PERSIST_CHUNK_SIZE}`);
        }
        const now = Date.now();
...
export const _markPickComplete = internalMutation({
    ...
    handler: async (ctx, args) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[_markPickComplete] no run ${args.runId}`);
        if (run.status !== "picking") {
            throw new Error(`[_markPickComplete] run ${args.runId} is ${run.status}, expected picking`);
        }
        const now = Date.now();
        await ctx.db.patch(run._id, {
...
export const _markPickFailed = internalMutation({
    args: {
        runId: v.string(),
        substatus: v.union(v.literal("empty-pool"), v.literal("pool-too-small"), v.literal("segment-create-failed"), v.literal("persist-failed")),
        error: v.string(),
    },
        ...
        if (substatus === "empty-pool" || substatus === "pool-too-small") {
            ...
            if (config && config.pendingRunId === runId) {
                await ctx.db.patch(config._id, {
                    ...
                    active: false,
                    lastRunStatus: substatus === "empty-pool"
...
export const pickWaveAction = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        if (!Number.isFinite(args.requestedCount) || args.requestedCount <= 0) {
            throw new Error(`[pickWaveAction] requestedCount must be a positive integer; got ${args.requestedCount}`);
        }
        ...
        const claim = await ctx.runMutation(internal.broadcast.waveRuns._claimWaveRunLease, {
            waveLabel: args.waveLabel,
            runId: args.runId,
            requestedCount: args.requestedCount,
            batchSize,
        ...
        if (!claim.ok) {
            throw new Error(`[pickWaveAction] could not claim lease: ${claim.reason}` +
        ...
        try {
            ...
            const suppressedSet = new Set(suppressed);
            const paidSet = new Set(paid);
            const reservoir = new Reservoir(args.requestedCount);
            ...
            while (true) {
                ...
                for (const email of result.eligible)
            ...
            await ctx.runMutation(internal.broadcast.waveRuns._recordPoolFilterStats, {
                runId: args.runId,
                excludeNonEnglish,
                eligiblePoolCount,
                excludedCount,
            ...
            if (picked.length < MIN_USABLE_POOL_SIZE) {
                await ctx.runMutation(internal.broadcast.waveRuns._markPickFailed, {
                    ...
                    error: `picked ${picked.length} contacts (< MIN_USABLE_POOL_SIZE=${MIN_USABLE_POOL_SIZE}); ` +
            ...
            await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.pushBatchAction, { runId: args.runId, batchN: 0 });
            console.log(`[pickWaveAction] complete: runId=${args.runId} waveLabel=${args.waveLabel} ` +
            ...
            // operator inspection).
            console.error(`[pickWaveAction] runId=${args.runId} unexpected error: ${err instanceof Error ? err.message : String(err)}`);
            throw err;
        }
    },
});
...
export const _markPushingStarted = internalMutation({
    ...
    handler: async (ctx, { runId }) => {
        ...
        if (run.status === "pushing")
            return { ok: true, alreadyPushing: true };
        if (run.status !== "segment-created") {
            return { ok: false, reason: `wrong-status-${run.status}` };
        }
        const now = Date.now();
        await ctx.db.patch(run._id, { status: "pushing", lastBatchAt: now, updatedAt: now });
        return { ok: true, alreadyPushing: false };
    },
});
/**
...
export const _markContactFailed = internalMutation({
    ...
    handler: async (ctx, { contactId, runId, normalizedEmail, failedReason }) => {
        ...
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            return { ok: true, runFailed: false };
        ...
        return { ok: true, runFailed: exceeded };
    },
});
export const pushBatchAction = internalAction({
    ...
    handler: async (ctx, { runId, batchN }) => {
        const apiKey = process.env.RESEND_API_KEY;
        if (!apiKey)
            throw new Error("[pushBatchAction] RESEND_API_KEY not set");
        // Lease + state revalidation.
        const info = await ctx.runQuery(internal.broadcast.waveRuns._resumeBatchInfo, { runId });
        if (!info) {
        ...
        const allowedStatuses = ["segment-created", "pushing"];
        if (!allowedStatuses.includes(info.run.status)) {
            console.warn(`[pushBatchAction] runId=${runId} status=${info.run.status} not pushable; exiting`);
        ...
        if (info.run.status === "segment-created") {
            await ctx.runMutation(internal.broadcast.waveRuns._markPushingStarted, { runId });
        }
        // Pull this batch's pending contacts.
        const batch = await ctx.runQuery(internal.broadcast.waveRuns._getPendingBatch, { runId, limit: info.run.batchSize });
        if (batch.length === 0) {
            // Nothing pending — schedule finalize.
            await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.finalizeWaveAction, { runId });
        ...
        for (const contact of batch) {
            ...
            if (result.kind === "failed") {
                const failResult = await ctx.runMutation(internal.broadcast.waveRuns._markContactFailed, {
                ...
                if (failResult.ok && failResult.runFailed) {
                    runFailed = true;
                    console.error(`[pushBatchAction] runId=${runId} batch=${batchN} failure-rate threshold tripped`);
                    break;
                }
                console.error(`[pushBatchAction] push failed for ${maskEmail(contact.normalizedEmail)}: ${result.reason}`);
                continue;
            }
            // Outcomes: created | linkedExisting | alreadyInSegment — all valid.
            await ctx.runMutation(internal.broadcast.waveRuns._markContactPushed, {
                contactId: contact._id,
                runId,
                normalizedEmail: contact.normalizedEmail,
        ...
        if (runFailed)
            return { ok: false, reason: "batch-failure-rate-exceeded" };
        // Decide next step from fresh state.
        const after = await ctx.runQuery(internal.broadcast.waveRuns._resumeBatchInfo, { runId });
        if (!after || after.run.status === "failed") {
            return { ok: false, reason: `terminal-status-${after?.run.status ?? "<missing>"}` };
        }
        if (after.hasPending) {
            await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.pushBatchAction, { runId, batchN: batchN + 1 });
...
export const _markBroadcastCreated = internalMutation({
    ...
    handler: async (ctx, { runId, broadcastId }) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[_markBroadcastCreated] no run ${runId}`);
...
export const _markFinalizeFailed = internalMutation({
    args: {
        runId: v.string(),
        substatus: v.union(v.literal("create-broadcast-failed"), v.literal("send-broadcast-failed")),
        error: v.string(),
    },
    handler: async (ctx, { runId, substatus, error }) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[_markFinalizeFailed] no run ${runId}`);
        ...
        const statusPatch = substatus === "create-broadcast-failed"
            ? { status: "failed" }
            : {};
        await ctx.db.patch(run._id, {
            ...statusPatch,
...
export const _finalizeWaveRun = internalMutation({
    ...
    handler: async (ctx, { runId, sentAt }) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[_finalizeWaveRun] no run ${runId}`);
        ...
        if (run.status === "sent") {
            ...
            return { ok: true, alreadySent: true, advancedToTier: undefined };
        }
        if (run.status !== "broadcast-created") {
            throw new Error(`[_finalizeWaveRun] run ${runId} is ${run.status}, expected broadcast-created`);
        }
        if (!run.broadcastId || !run.segmentId) {
            throw new Error(`[_finalizeWaveRun] run ${runId} missing broadcastId/segmentId`);
        }
        const config = await ctx.db
            .query("broadcastRampConfig")
            .withIndex("by_key", (q) => q.eq("key", "current"))
            .unique();
        if (!config)
            throw new Error("[_finalizeWaveRun] no broadcastRampConfig");
        if (config.pendingRunId !== runId) {
            throw new Error(`[_finalizeWaveRun] lost lease: expected ${runId}, found ${config.pendingRunId ?? "<cleared>"}. ` +
        ...
        return { ok: true, advancedToTier: nextTier };
    },
});
export const finalizeWaveAction = internalAction({
    ...
    handler: async (ctx, { runId }) => {
        ...
        if (info.run.status === "pushing" || info.run.status === "segment-created") {
            let createResult;
            try {
                createResult = await ctx.runAction(internal.broadcast.sendBroadcast.createProLaunchBroadcast, {
                    segmentId: info.run.segmentId,
                    nameSuffix: info.run.waveLabel,
                });
            }
            catch (err) {
                await ctx.runMutation(internal.broadcast.waveRuns._markFinalizeFailed, {
                    runId,
            ...
            const markResult = await ctx.runMutation(internal.broadcast.waveRuns._markBroadcastCreated, { runId, broadcastId: createResult.broadcastId });
            if (!markResult.ok) {
                console.error(`[finalizeWaveAction] CAS lost on _markBroadcastCreated runId=${runId} reason=${markResult.reason} ` +
        ...
        try {
            await ctx.runAction(internal.broadcast.sendBroadcast.sendProLaunchBroadcast, { broadcastId: after.run.broadcastId });
        }
        catch (err) {
            const failResult = await ctx.runMutation(internal.broadcast.waveRuns._markFinalizeFailed, {
                runId,
            ...
            if (!failResult.ok && failResult.reason === "already-sent") {
                console.log(`[finalizeWaveAction] runId=${runId} send returned error but run already sent ` +
...
export const markFinalizeRecovered = internalMutation({
    ...
    handler: async (ctx, { runId, sentAt, reason }) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[markFinalizeRecovered] no run ${runId}`);
        ...
        if (run.status !== "broadcast-created") {
            throw new Error(`[markFinalizeRecovered] run ${runId} status=${run.status} — recovery requires status='broadcast-created'. ` +
                (run.status === "sent"
        ...
        if (run.failureSubstatus !== "send-broadcast-failed") {
            throw new Error(`[markFinalizeRecovered] run ${runId} status='broadcast-created' but failureSubstatus=` +
        ...
        const config = await ctx.db
            ...
            .unique();
        if (!config)
            throw new Error("[markFinalizeRecovered] no broadcastRampConfig");
        ...
        if (config.pendingRunId !== runId) {
            throw new Error(`[markFinalizeRecovered] runId=${runId} lost lease (held by ${config.pendingRunId ?? "<cleared>"}). ` +
...
export const discardWaveRun = internalMutation({
    ...
    handler: async (ctx, { runId, reason }) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[discardWaveRun] no run ${runId}`);
        const config = await ctx.db
            .query("broadcastRampConfig")
            .withIndex("by_key", (q) => q.eq("key", "current"))
            .unique();
        if (!config)
            throw new Error("[discardWaveRun] no broadcastRampConfig");
        const now = Date.now();
        await ctx.db.patch(run._id, {
            status: "failed",
        ...
        await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.cleanupDiscardedWavePickedContactsAction, { runId });
        return {
            ok: true,
...
export const resumeStalledWaveRun = internalMutation({
    ...
    handler: async (ctx, { runId }) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[resumeStalledWaveRun] no run ${runId}`);
        if (run.status === "broadcast-created") {
            throw new Error(`[resumeStalledWaveRun] runId=${runId} is in broadcast-created — use resumeFinalizeWaveRun({confirmedNotSent: true}) after Resend-dashboard verification, OR markFinalizeRecovered if the broadcast was actually sent.`);
        }
        if (run.status === "failed") {
            throw new Error(`[resumeStalledWaveRun] runId=${runId} is in failed (substatus=${run.failureSubstatus ?? "<none>"}) — use resumeFinalizeWaveRun (for create/send substatuses) or discardWaveRun (for batch-failure-rate-exceeded / pick-phase substatuses).`);
        }
        if (run.status === "sent") {
        ...
        await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.pushBatchAction, { runId, batchN: 0 });
        return { ok: true, scheduled: "pushBatchAction" };
    },
});
/**
...
export const resumeFinalizeWaveRun = internalMutation({
    ...
    handler: async (ctx, { runId, confirmedNotSent }) => {
        const run = await ctx.db
            ...
            .unique();
        if (!run)
            throw new Error(`[resumeFinalizeWaveRun] no run ${runId}`);
        const isSendFailureCase = run.status === "broadcast-created" ||
            run.failureSubstatus === "send-broadcast-failed";
        const isCreateFailureCase = run.status === "failed" &&
            run.failureSubstatus === "create-broadcast-failed";
        if (isSendFailureCase) {
            if (confirmedNotSent !== true) {
                throw new Error(`[resumeFinalizeWaveRun] runId=${runId} is in send-failure state. ` +
            ...
            await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.finalizeWaveAction, { runId });
            return { ok: true, scheduled: "finalizeWaveAction-send-only" };
        }
        if (isCreateFailureCase) {
            ...
            await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.finalizeWaveAction, { runId });
            return { ok: true, scheduled: "finalizeWaveAction-create-and-send" };
        }
        throw new Error(`[resumeFinalizeWaveRun] runId=${runId} is in status=${run.status} substatus=${run.failureSubstatus ?? "<none>"} — ` +
    ...
    handler: async (ctx, { runId }) => {
        ...
        for (const row of rows) {
            if (row.status === "pushed" && waveLabel) {
                const reg = await ctx.db
                    .query("registrations")
                    .withIndex("by_normalized_email", (q) => q.eq("normalizedEmail", row.normalizedEmail))
                    .first();
...
export const cleanupDiscardedWavePickedContactsAction = internalAction({
    ...
    handler: async (ctx, args) => {
        if (args.runId) {
            const result = await ctx.runMutation(internal.broadcast.waveRuns._cleanupDiscardedWavePickedContacts, { runId: args.runId });
            if (result.hasMore) {
                await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.cleanupDiscardedWavePickedContactsAction, { runId: args.runId });
            }
            else if (result.unstamped > 0) {
                console.log(`[cleanupDiscardedWavePickedContactsAction] runId=${args.runId} ` +
                    `unstamped ${result.unstamped} registrations (re-eligible for future picks)`);
            }
            return result;
        }
        // No specific runId — scan failed runs >24h old.
        const candidates = await ctx.runQuery(internal.broadcast.waveRuns._listFailedWaveRunsForCleanup, {});
        let totalDeleted = 0;
        let totalUnstamped = 0;
        for (const runId of candidates) {
            const result = await ctx.runMutation(internal.broadcast.waveRuns._cleanupDiscardedWavePickedContacts, { runId });
...
export const _listFailedWaveRunsForCleanup = internalQuery({
    ...
    handler: async (ctx) => {
        ...
        return failed
            ...
            TERMINAL_FAILURE_SUBSTATUSES.includes(r.failureSubstatus))
            .map((r) => r.runId);
    },
});
// ───────────────────────────────────────────────────────────────────────────

 ...
 */
export function isDuplicateContactError(body) {
    ...
    if (name.includes("already_exists") || name.includes("duplicate"))
        return true;
 ...
 */
export async function upsertContactToSegment(apiKey, email, segmentId) {
    ...
    if (createRes.status === 422) {
        ...
        const addRes = await fetch(`${RESEND_API_BASE}/contacts/${encodeURIComponent(email)}/segments/${encodeURIComponent(segmentId)}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${apiKey}`,
        ...
        if (addRes.ok)
            return { kind: "linkedExisting" };
        if (addRes.status === 422) {
            const addBody = await addRes.json().catch(() => null);
            if (isDuplicateContactError(addBody))
                return { kind: "alreadyInSegment" };
            return {
                kind: "failed",
 ...
 * `pro-launch-wave-2`).
 */
export async function createSegment(apiKey, name) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`[createSegment] Resend POST /segments ${res.status}: ${body}`);
    }
    const json = (await res.json().catch(() => null));
    if (!json?.id) {
        throw new Error(`[createSegment] Resend response missing id: ${JSON.stringify(json)}`);
    }
    return json.id;
}

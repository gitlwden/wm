...
export const exportProLaunchAudience = internalAction({
    ...
    handler: async (ctx, { segmentId, cursor, numItems, dryRun }) => {
        const apiKey = process.env.RESEND_API_KEY;
        if (!apiKey && !dryRun) {
            throw new Error("[exportProLaunchAudience] RESEND_API_KEY not set (omit or set dryRun:true to test without sending)");
        }
        ...
        const page = await ctx.runQuery(internal.broadcast.audienceExport.getRegistrationsPage, { cursor: cursor ?? null, numItems: pageSize });
        const stats = {
            upserted: 0,
            linkedExisting: 0,
            alreadyExists: 0,
        ...
        for (const row of page.page) {
            ...
            switch (outcome.kind) {
                ...
                case "failed":
                    ...
                    // land there.
                    console.error(`[exportProLaunchAudience] Resend failure for ${maskEmail(email)}: ${outcome.reason}`);
                    break;
            }
        }
        console.log(`[exportProLaunchAudience] page complete: ${JSON.stringify(stats)}`);
        return stats;
    },
});

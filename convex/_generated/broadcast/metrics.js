...
const TRACKED_SET = new Set(BROADCAST_TRACKED_EVENT_TYPES);
/**
...
export const recordBroadcastEvent = internalMutation({
    ...
    handler: async (ctx, args) => {
        if (!TRACKED_SET.has(args.eventType)) {
            ...
            return { inserted: false, reason: "untracked_event_type" };
        }
        const existing = await ctx.db
            .query("broadcastEvents")
            .withIndex("by_webhookEventId", (q) => q.eq("webhookEventId", args.webhookEventId))
            .first();
        if (existing) {
...
export const _countBroadcastEventsPage = internalQuery({
    ...
    handler: async (ctx, { broadcastId, eventType, cursor }) => {
        const result = await ctx.db
            .query("broadcastEvents")
            .withIndex("by_broadcast_event", (q) => q.eq("broadcastId", broadcastId).eq("eventType", eventType))
...
export const getBroadcastStats = internalAction({
    ...
    handler: async (ctx, { broadcastId }) => {
        ...
        for (const eventType of BROADCAST_TRACKED_EVENT_TYPES) {
            ...
            while (true) {
                const page = await ctx.runQuery(internal.broadcast.metrics._countBroadcastEventsPage, { broadcastId, eventType, cursor });
                total += page.count;
                if (page.isDone)
                    break;
        ...
        const rate = (n) => (delivered > 0 ? n / delivered : null);
        ...
        return {
            ...
            bouncesOverThreshold: bounceRate !== null && bounceRate > BOUNCE_KILL_THRESHOLD,
            complaintsOverThreshold: complaintRate !== null && complaintRate > COMPLAINT_KILL_THRESHOLD,
        };
    },
});

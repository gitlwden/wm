...
export const createProLaunchBroadcast = internalAction({
    ...
    handler: async (_ctx, { segmentId, nameSuffix }) => {
        ...
        if (PRO_LAUNCH_PHYSICAL_ADDRESS.includes("TBD") ||
            PRO_LAUNCH_PHYSICAL_ADDRESS.includes("placeholder")) {
            throw new Error(`[createProLaunchBroadcast] PRO_LAUNCH_PHYSICAL_ADDRESS is still a placeholder ("${PRO_LAUNCH_PHYSICAL_ADDRESS}"). ` +
                "Set the real CAN-SPAM postal address in convex/broadcast/proLaunchEmailContent.ts before sending.");
        }
        const name = nameSuffix && nameSuffix.length > 0
        ...
        if (!res.ok) {
            ...
            throw new Error(`[createProLaunchBroadcast] Resend ${res.status}: ${body}`);
        }
        const json = (await res.json().catch(() => null));
        if (!json?.id) {
            throw new Error(`[createProLaunchBroadcast] Resend response missing id: ${JSON.stringify(json)}`);
        }
        return {
            broadcastId: json.id,
            name,
...
export const sendProLaunchBroadcast = internalAction({
    ...
    handler: async (_ctx, { broadcastId, scheduledAt }) => {
        ...
        const res = await fetch(`${RESEND_API_BASE}/broadcasts/${encodeURIComponent(broadcastId)}/send`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${apiKey}`,
        ...
        if (!res.ok) {
            ...
            throw new Error(`[sendProLaunchBroadcast] Resend ${res.status}: ${errBody}`);
        }
        return {
            broadcastId,

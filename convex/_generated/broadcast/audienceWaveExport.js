...
export const assignAndExportWave = internalAction({
    ...
    handler: async (ctx, { waveLabel, count }) => {
        const apiKey = process.env.RESEND_API_KEY;
        if (!apiKey) {
            throw new Error("[assignAndExportWave] RESEND_API_KEY not set — run with the same env that hosts the prior segments.");
        }
        if (!Number.isFinite(count) || count <= 0) {
            throw new Error(`[assignAndExportWave] count must be a positive integer; got ${count}`);
        }
        if (waveLabel.length === 0 || waveLabel.length > 64) {
            throw new Error("[assignAndExportWave] waveLabel must be 1-64 chars");
        }
        ...
        if (exists) {
            throw new Error(`[assignAndExportWave] waveLabel "${waveLabel}" already has stamped rows — pick a unique label, or use the Resend dashboard to inspect/clean up the prior wave's segment first.`);
        }
        // Step 2: stream registrations, dedup, reservoir-sample N.
        const [suppressed, paid] = await Promise.all([
            ctx.runQuery(internal.broadcast.audienceWaveExport._getSuppressedEmails, {}),
            ctx.runQuery(internal.broadcast.audienceWaveExport._getPaidEmails, {}),
        ]);
        const suppressedSet = new Set(suppressed);
        const paidSet = new Set(paid);
        const reservoir = new Reservoir(count);
        let cursor = null;
        while (true) {
            ...
            // tsconfig change.
            const page = await ctx.runQuery(internal.broadcast.audienceWaveExport._getRegistrationsPage, { cursor, numItems: REGISTRATIONS_PAGE_SIZE });
        ...
        if (picked.length === 0) {
            throw new Error(`[assignAndExportWave] pool empty — all registrations are suppressed/paid/already-stamped. Nothing to send.`);
        }
        ...
        for (const email of picked) {
            ...
            switch (outcome.kind) {
                ...
                case "failed":
                    ...
                    // not land there.
                    console.error(`[assignAndExportWave] Resend push failed for ${maskEmail(email)}: ${outcome.reason}`);
            ...
            try {
                await ctx.runMutation(internal.broadcast.audienceWaveExport._stampWaveByNormalizedEmail, { normalizedEmail: email, waveLabel, assignedAt });
            }
            catch (err) {
                ...
                stats.stampFailed++;
                console.error(`[assignAndExportWave] stamp failed for ${maskEmail(email)} (already in Resend segment ${segmentName}): ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        console.log(`[assignAndExportWave] complete: ${JSON.stringify(stats)}`);
        return stats;
    },
});

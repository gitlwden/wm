...
export const backfillCanary250 = internalAction({
    ...
    handler: async (ctx) => {
        ...
        if (!apiKey) {
            throw new Error("[backfillCanary250] RESEND_API_KEY not set — run with the same env that hosts the canary segment.");
        }
        if (!Number.isFinite(CANARY_SENT_AT_MS)) {
            throw new Error("[backfillCanary250] CANARY_SENT_AT_MS failed Date.parse — check the literal.");
        }
        const stats = {
            fetched: 0,
            stamped: 0,
        ...
        while (true) {
            ...
            const url = new URL(`${RESEND_API_BASE}/segments/${encodeURIComponent(CANARY_SEGMENT_ID)}/contacts`);
            ...
            if (!res.ok) {
                ...
                throw new Error(`[backfillCanary250] Resend list-contacts ${res.status}: ${body}`);
            }
            const json = (await res.json());
            if (!json || !Array.isArray(json.data)) {
                throw new Error(`[backfillCanary250] unexpected Resend response shape: ${JSON.stringify(json).slice(0, 200)}`);
            }
            for (const contact of json.data) {
                ...
                try {
                    const out = await ctx.runMutation(internal.broadcast.backfillCanaryWaveStamps
                        ._stampWaveByNormalizedEmail, {
                        normalizedEmail,
                        waveLabel: CANARY_WAVE_LABEL,
                        assignedAt: CANARY_SENT_AT_MS,
                    });
                    if (out.result === "stamped")
                        stats.stamped++;
                    else if (out.result === "alreadyStamped")
                        ...
                        console.log(`[backfillCanary250] no registration for ${maskEmail(normalizedEmail)}`);
                    }
                }
                catch (err) {
                    ...
                    stats.failed++;
                    console.error(`[backfillCanary250] stamp failed for ${maskEmail(normalizedEmail)}:`, err instanceof Error ? err.message : String(err));
                }
            }
        ...
        console.log(`[backfillCanary250] complete: ${JSON.stringify(stats)}`);
        return stats;
    },
});

...
export const initRamp = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (hasSeedBroadcast !== hasSeedSentAt) {
            throw new Error("[initRamp] seedLastWaveBroadcastId and seedLastWaveSentAt must be provided together.");
        }
        if (offset > 0 && !hasSeedBroadcast) {
            throw new Error(`[initRamp] waveLabelOffset=${offset} signals resumption after manual waves; seedLastWaveBroadcastId + seedLastWaveSentAt are required so the first cron tick can apply the kill-gate against the prior wave. Pass them, or set waveLabelOffset=0 to start a fresh ramp.`);
        }
        const existing = await ctx.db
            ...
            .first();
        if (existing) {
            throw new Error(`[initRamp] ramp already configured (active=${existing.active}, tier=${existing.currentTier}). Run abortRamp first if reconfiguring.`);
        }
        await ctx.db.insert("broadcastRampConfig", {
            ...
            bounceKillThreshold: args.bounceKillThreshold ?? DEFAULT_BOUNCE_KILL_THRESHOLD,
            complaintKillThreshold: args.complaintKillThreshold ?? DEFAULT_COMPLAINT_KILL_THRESHOLD,
...
export const pauseRamp = internalMutation({
    ...
    handler: async (ctx) => {
        ...
        if (!row)
            throw new Error("[pauseRamp] no ramp configured");
...
export const resumeRamp = internalMutation({
    ...
    handler: async (ctx) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[resumeRamp] no ramp configured");
        if (row.killGateTripped) {
            throw new Error("[resumeRamp] kill-gate is tripped; clearKillGate first after investigating.");
        }
...
export const clearKillGate = internalMutation({
    ...
    handler: async (ctx, { reason }) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[clearKillGate] no ramp configured");
        if (!row.killGateTripped) {
...
export const clearPartialFailure = internalMutation({
    ...
    handler: async (ctx, { reason }) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[clearPartialFailure] no ramp configured");
        ...
        if (row.pendingWaveLabel ||
            row.pendingSegmentId ||
            row.pendingBroadcastId) {
            throw new Error(`[clearPartialFailure] refused: pending progress markers present (waveLabel=${row.pendingWaveLabel ?? "-"}, segmentId=${row.pendingSegmentId ?? "-"}, broadcastId=${row.pendingBroadcastId ?? "-"}). The export DID run; clearing here would mask stamped contacts. Use recoverFromPartialFailure instead.`);
        }
        await ctx.db.patch(row._id, {
...
export const recoverFromPartialFailure = internalMutation({
    args: {
        recovery: v.union(v.literal("manual-finished"), v.literal("discard-and-rotate")),
        reason: v.string(),
    ...
    handler: async (ctx, args) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[recoverFromPartialFailure] no ramp configured");
        if (row.lastRunStatus !== "partial-failure") {
            return {
                ok: true,
                noop: true,
        ...
        if (args.recovery === "manual-finished") {
            ...
            if (nextTier >= row.rampCurve.length) {
                throw new Error(`[recoverFromPartialFailure:manual-finished] currentTier=${row.currentTier} would advance past rampCurve.length=${row.rampCurve.length}. Curve is complete; nothing to recover.`);
            }
            const waveLabel = args.waveLabel ??
                row.pendingWaveLabel ??
                `${row.waveLabelPrefix}-${nextTier + row.waveLabelOffset}`;
            const missing = [];
            if (!broadcastId)
                missing.push("broadcastId");
            if (!segmentId)
                missing.push("segmentId");
            if (assigned === undefined)
                missing.push("assigned");
            if (args.sentAt === undefined)
                missing.push("sentAt");
            if (missing.length > 0) {
                throw new Error(`[recoverFromPartialFailure:manual-finished] missing required field(s): ${missing.join(", ")}. ` +
            ...
            return {
                ...
                usedPersistedFallback: {
                    ...
                    assigned: args.assigned === undefined && row.pendingAssigned !== undefined,
...
export const getRampStatus = internalQuery({
    ...
    handler: async (ctx) => {
        ...
        const nextTier = row.currentTier + 1;
        const nextWaveLabel = nextTier < row.rampCurve.length
            ? `${row.waveLabelPrefix}-${nextTier + row.waveLabelOffset}`
            : null;
        const nextWaveCount = nextTier < row.rampCurve.length ? row.rampCurve[nextTier] : null;
        return {
            configured: true,
            active: row.active,
            ...
            leaseAgeWarn: row.pendingRunId !== undefined &&
            ...
            activeWaveRuns: await (async () => {
                const runs = [];
                const activeStatuses = ["picking", "segment-created", "pushing", "broadcast-created"];
    ...
    handler: async (ctx, args) => {
        ...
        if (row.pendingRunId) {
            return {
                ...
                ageMs: row.pendingRunStartedAt !== undefined
...
export const forceReleaseLease = internalMutation({
    ...
    handler: async (ctx, { reason }) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[forceReleaseLease] no ramp configured");
        if (!row.pendingRunId) {
            return {
                ok: true,
                noop: true,
        ...
        const heldForMs = row.pendingRunStartedAt !== undefined
...
export const _recordPendingExport = internalMutation({
    ...
    handler: async (ctx, args) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[_recordPendingExport] no ramp configured");
        if (row.pendingRunId !== args.runId) {
            throw new Error(`[_recordPendingExport] lease lost: expected runId=${args.runId}, found ${row.pendingRunId ?? "<cleared>"}. Refusing to persist export progress — operator/another run owns the state.`);
        }
        await ctx.db.patch(row._id, {
...
export const _recordPendingBroadcast = internalMutation({
    ...
    handler: async (ctx, args) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[_recordPendingBroadcast] no ramp configured");
        if (row.pendingRunId !== args.runId) {
            throw new Error(`[_recordPendingBroadcast] lease lost: expected runId=${args.runId}, found ${row.pendingRunId ?? "<cleared>"}. Refusing to persist broadcast progress — operator/another run owns the state.`);
        }
        await ctx.db.patch(row._id, {
...
export const _recordWaveSent = internalMutation({
    ...
    handler: async (ctx, args) => {
        const row = await loadConfig(ctx);
        if (!row)
            throw new Error("[_recordWaveSent] no ramp configured");
        if (row.currentTier !== args.expectedCurrentTier) {
            throw new Error(`[_recordWaveSent] tier moved underneath us: expected ${args.expectedCurrentTier}, found ${row.currentTier}. Refusing to overwrite.`);
        }
        if (row.pendingRunId !== args.runId) {
            ...
            throw new Error(`[_recordWaveSent] lease lost: expected runId=${args.runId}, found ${row.pendingRunId ?? "<cleared>"}. Refusing to advance tier — investigate what cleared the lease.`);
        }
        await ctx.db.patch(row._id, {
...
export const _recordRunOutcome = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.runId && row.pendingRunId !== args.runId) {
            console.warn(`[_recordRunOutcome] lease lost (expected ${args.runId}, found ${row.pendingRunId ?? "<cleared>"}); refusing to write outcome status=${args.status}.`);
            return {
                ok: false,
                reason: "lease-lost",
...
export const runDailyRamp = internalAction({
    args: {},
    handler: async (ctx) => {
        const row = await ctx.runQuery(internal.broadcast.rampRunner._loadConfigForRunner, {});
        if (!row) {
        ...
        if (row.killGateTripped) {
            console.log(`[runDailyRamp] kill-gate tripped (${row.killGateReason ?? "<no reason>"}) — skip`);
            return { status: "kill-gate-tripped" };
        }
        if (row.lastRunStatus === "partial-failure") {
            console.log("[runDailyRamp] last run was a partial failure — skip until operator clears.");
        ...
        if (row.lastWaveBroadcastId) {
            ...
            const hoursSince = (Date.now() - (row.lastWaveSentAt ?? 0)) / (1000 * 60 * 60);
            if (hoursSince < MIN_HOURS_BETWEEN_WAVES) {
                console.log(`[runDailyRamp] only ${hoursSince.toFixed(1)}h since last wave (need ${MIN_HOURS_BETWEEN_WAVES}h) — skip`);
                await ctx.runMutation(internal.broadcast.rampRunner._recordRunOutcome, { status: "awaiting-prior-stats" });
                return { status: "awaiting-prior-stats" };
            }
            const stats = await ctx.runAction(internal.broadcast.metrics.getBroadcastStats, { broadcastId: row.lastWaveBroadcastId });
            const delivered = stats.counts["email.delivered"] ?? 0;
            if (delivered < MIN_DELIVERED_FOR_KILLGATE) {
                console.log(`[runDailyRamp] prior wave only ${delivered} delivered (need ${MIN_DELIVERED_FOR_KILLGATE}) — skip`);
                await ctx.runMutation(internal.broadcast.rampRunner._recordRunOutcome, { status: "awaiting-prior-stats" });
                return { status: "awaiting-prior-stats" };
            }
            if (stats.bounceRate !== null &&
                ...
                await ctx.runMutation(internal.broadcast.rampRunner._recordRunOutcome, {
        ...
        if (nextTier >= row.rampCurve.length) {
            ...
            await ctx.runMutation(internal.broadcast.rampRunner._recordRunOutcome, { status: "ramp-complete", deactivate: true });
        ...
        if (count === undefined) {
            throw new Error(`[runDailyRamp] rampCurve[${nextTier}] is undefined despite bounds check — config corruption?`);
        }
        ...
        // active waveRuns row.
        const inFlight = await ctx.runQuery(internal.broadcast.waveRuns._listInFlightWaveRuns, {});
        if (inFlight.length > 0) {
            ...
            if (!top)
                throw new Error("[runDailyRamp] inFlight bounds-check invariant violated");
            const ageMin = (Date.now() - top.lastActivityAt) / 60_000;
            if (ageMin >= 15) {
                console.error(`[runDailyRamp] STALLED waveRun runId=${top.runId} status=${top.status} (last activity ${ageMin.toFixed(1)}min ago) — operator must resume (resumeStalledWaveRun / resumeFinalizeWaveRun) or discard (discardWaveRun) before next tick.`);
                return { status: "stalled-wave-run", detail: top.runId };
            }
            console.log(`[runDailyRamp] wave already in flight (runId=${top.runId} status=${top.status}, last activity ${ageMin.toFixed(1)}min ago) — skip`);
        ...
        if (row.pendingRunId) {
            ...
            const heldRun = await ctx.runQuery(internal.broadcast.waveRuns.getWaveRunStatus, { runId: row.pendingRunId });
            const detail = heldRun !== null
                ? `${row.pendingRunId} status=${heldRun.status}` +
                    (heldRun.failureSubstatus ? ` substatus=${heldRun.failureSubstatus}` : "")
                : row.pendingRunId;
            console.error(`[runDailyRamp] LEASE HELD by ${detail} — likely a failed run pending operator action ` +
        ...
        await ctx.scheduler.runAfter(0, internal.broadcast.waveRuns.pickWaveAction, { waveLabel, runId, requestedCount: count, excludeNonEnglish });
        console.log(`[runDailyRamp] scheduled pickWaveAction runId=${runId} waveLabel=${waveLabel} requestedCount=${count} excludeNonEnglish=${excludeNonEnglish}`);

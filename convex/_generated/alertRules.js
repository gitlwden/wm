 ...
 */
async function assertProEntitlement(ctx, userId) {
    ...
    if (tier < 1) {
        throw new ConvexError({
            code: "PRO_REQUIRED",
            message: "Notifications are a PRO feature. Upgrade to enable real-time and digest alerts.",
        });
    }
}
...
function assertCompatibleDeliveryMode(pair) {
    if (pair.digestMode === "realtime" && (pair.sensitivity === "all" || pair.sensitivity === "high")) {
        throw new ConvexError({
            code: "INCOMPATIBLE_DELIVERY",
            message: "Real-time delivery is for Critical events only. " +
...
export const setAlertRules = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("alertRules")
            .withIndex("by_user_variant", (q) => q.eq("userId", userId).eq("variant", args.variant))
            .unique();
...
export const setAlertRulesForUser = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("alertRules")
            .withIndex("by_user_variant", (q) => q.eq("userId", userId).eq("variant", rest.variant))
            .unique();
        const pair = resolveEffectivePair({
...
export const setNotificationConfigForUser = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (existing) {
            ...
            if (args.aiDigestEnabled !== undefined)
                patch.aiDigestEnabled = args.aiDigestEnabled;
            if (args.digestMode !== undefined)
                patch.digestMode = args.digestMode;
...
export const getDigestRules = internalQuery({
    ...
    handler: async (ctx) => {
        const enabled = await ctx.db
            ...
            .collect();
        return enabled.filter((r) => r.digestMode !== undefined && r.digestMode !== "realtime");
    },
});

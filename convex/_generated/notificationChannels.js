...
export const setChannelForUser = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("notificationChannels")
            .withIndex("by_user_channel", (q) => q.eq("userId", userId).eq("channelType", channelType))
            .unique();
        const isNew = !existing;
        const now = Date.now();
        if (channelType === "telegram") {
            if (!chatId)
                throw new ConvexError("chatId required for telegram channel");
            const doc = { userId, channelType: "telegram", chatId, verified: true, linkedAt: now };
            if (existing) {
            ...
            if (!webhookEnvelope)
                throw new ConvexError("webhookEnvelope required for slack channel");
            const doc = { userId, channelType: "slack", webhookEnvelope, verified: true, linkedAt: now };
            if (existing) {
                await ctx.db.replace(existing._id, doc);
            }
            else {
            ...
            if (!email)
                throw new ConvexError("email required for email channel");
            const doc = { userId, channelType: "email", email, verified: true, linkedAt: now };
            if (existing) {
                await ctx.db.replace(existing._id, doc);
            }
            else {
            ...
            if (!webhookEnvelope)
                throw new ConvexError("webhookEnvelope required for webhook channel");
            const doc = { userId, channelType: "webhook", webhookEnvelope, verified: true, linkedAt: now, webhookLabel };
            if (existing) {
                await ctx.db.replace(existing._id, doc);
            }
            else {
...
export const setWebPushChannelForUser = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("notificationChannels")
            .withIndex("by_user_channel", (q) => q.eq("userId", args.userId).eq("channelType", "web_push"))
            .unique();
        const isNew = !existing;
        const doc = {
            userId: args.userId,
...
export const setSlackOAuthChannelForUser = internalMutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            .query("notificationChannels")
            .withIndex("by_user_channel", (q) => q.eq("userId", args.userId).eq("channelType", "slack"))
            .unique();
        const isNew = !existing;
        const doc = {
            userId: args.userId,
...
export const setDiscordOAuthChannelForUser = internalMutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            .query("notificationChannels")
            .withIndex("by_user_channel", (q) => q.eq("userId", args.userId).eq("channelType", "discord"))
            .unique();
        const isNew = !existing;
        const doc = {
            userId: args.userId,
...
export const deleteChannelForUser = internalMutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            .query("notificationChannels")
            .withIndex("by_user_channel", (q) => q.eq("userId", args.userId).eq("channelType", args.channelType))
            .unique();
        if (!existing)
            return;
...
export const setChannel = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("notificationChannels")
            .withIndex("by_user_channel", (q) => q.eq("userId", userId).eq("channelType", args.channelType))
            .unique();
        const now = Date.now();
        if (args.channelType === "telegram") {
            if (!args.chatId)
                throw new ConvexError("chatId required for telegram channel");
            const doc = { userId, channelType: "telegram", chatId: args.chatId, verified: true, linkedAt: now };
            if (existing) {
                await ctx.db.replace(existing._id, doc);
            }
            else {
            ...
            if (!args.webhookEnvelope)
                throw new ConvexError("webhookEnvelope required for slack channel");
            const doc = { userId, channelType: "slack", webhookEnvelope: args.webhookEnvelope, verified: true, linkedAt: now };
            if (existing) {
                await ctx.db.replace(existing._id, doc);
            }
            else {
            ...
            if (!args.email)
                throw new ConvexError("email required for email channel");
            const doc = { userId, channelType: "email", email: args.email, verified: true, linkedAt: now };
            if (existing) {
                await ctx.db.replace(existing._id, doc);
            }
            else {
            ...
            if (!args.webhookEnvelope)
                throw new ConvexError("webhookEnvelope required for webhook channel");
            const doc = { userId, channelType: "webhook", webhookEnvelope: args.webhookEnvelope, verified: true, linkedAt: now, webhookLabel: args.webhookLabel };
            if (existing) {
                await ctx.db.replace(existing._id, doc);
            }
            else {
...
export const claimPairingToken = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("notificationChannels")
            .withIndex("by_user_channel", (q) => q.eq("userId", record.userId).eq("channelType", "telegram"))
            .unique();
        const doc = {
        ...
        if (!existing) {
            const rules = await (record.variant
                ? ctx.db
                    .query("alertRules")
                    .withIndex("by_user_variant", (q) => q.eq("userId", record.userId).eq("variant", record.variant))
                    .collect()
                : ctx.db
                    .query("alertRules")

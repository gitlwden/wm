...
function allowedOrigin(origin, trusted) {
    if (!origin)
        return null;
...
async function timingSafeEqualStrings(a, b) {
    ...
    const keyMaterial = await crypto.subtle.generateKey({ name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
 ...
 */
function extractConvexErrorCode(err) {
    const raw = err?.data;
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        const result = await ctx.runQuery(internal.entitlements.getEntitlementsByUserId, { userId: body.userId });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        try {
            const result = (await ctx.runMutation(anyApi.userPreferences.setPreferences, {
            ...
            return new Response(JSON.stringify({ syncVersion: result.syncVersion }), { status: 200, headers });
        }
        catch (err) {
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        if (!provided) {
            ...
            console.warn("[telegram-webhook] secret header absent — rejecting request");
            return new Response("OK", { status: 200 });
        }
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        try {
            ...
            if (action === "set-web-push") {
                ...
                const webPushResult = await ctx.runMutation(internal.notificationChannels.setWebPushChannelForUser, {
                    userId,
            ...
            if (action === "set-alert-rules") {
                ...
                if (typeof body.variant !== "string" || !body.variant ||
                    ...
                    (body.sensitivity !== undefined && !VALID_SENSITIVITY.has(body.sensitivity)) ||
                ...
                await ctx.runMutation(internal.alertRules.setAlertRulesForUser, {
                    ...
                    countries: Array.isArray(body.countries) ? body.countries : undefined,
                });
                return new Response(JSON.stringify({ ok: true }), { status: 200, headers: { "Content-Type": "application/json" } });
            }
            if (action === "set-quiet-hours") {
                ...
                await ctx.runMutation(internal.alertRules.setQuietHoursForUser, {
                    ...
                    quietHoursOverride: body.quietHoursOverride,
                    countries: Array.isArray(body.countries) ? body.countries : undefined,
                });
                return new Response(JSON.stringify({ ok: true }), { status: 200, headers: { "Content-Type": "application/json" } });
            }
            if (action === "set-digest-settings") {
                ...
                if (typeof body.variant !== "string" || !body.variant ||
                    !VALID_DIGEST_MODE.has(body.digestMode)) {
                    return new Response(JSON.stringify({ error: "MISSING_REQUIRED_FIELDS" }), { status: 400, headers: { "Content-Type": "application/json" } });
                }
                ...
                try {
                    ...
                    if (data && typeof data === "object") {
                        ...
                        if (errPayload.code === "INCOMPATIBLE_DELIVERY") {
                            return new Response(JSON.stringify({ error: errPayload.code, message: errPayload.message ?? "" }), { status: 400, headers: { "Content-Type": "application/json" } });
                        }
                        if (errPayload.code === "PRO_REQUIRED") {
                            ...
                            // failure toast.
                            return new Response(JSON.stringify({ error: errPayload.code, message: errPayload.message ?? "" }), { status: 402, headers: { "Content-Type": "application/json" } });
                        }
                    }
                    throw err;
                }
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        const prefs = await ctx.runQuery(internal.userPreferences.getPreferencesByUserId, { userId: body.userId, variant: body.variant });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        const countries = await ctx.runQuery(internal.followedCountries.internalListFollowedForUser, { userId: body.userId });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        const ent = await ctx.runQuery(internal.entitlements.getEntitlementsByUserId, { userId: body.userId });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        const result = await ctx.runMutation(internal.registerInterest.registerUserReferralCode, { userId, code });
        return new Response(JSON.stringify(result), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        const result = await ctx.runQuery(internal.apiKeys.validateKeyByHash, { keyHash: body.keyHash });
        if (result) {
            try {
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        const result = await ctx.runQuery(internal.apiKeys.getKeyOwner, { keyHash: body.keyHash });
        return new Response(JSON.stringify(result), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        try {
            const result = await ctx.runMutation(internal.mcpProTokens.issueProMcpToken, {
                userId: body.userId,
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        try {
            const result = await ctx.runQuery(internal.mcpProTokens.validateProMcpToken, { tokenId: body.tokenId });
            if (result) {
                try {
                    await ctx.scheduler.runAfter(0, internal.mcpProTokens.touchProMcpTokenLastUsed, { tokenId: body.tokenId });
                }
                catch (err) {
                    // sentry-coverage-ok: best-effort lastUsedAt bump; mirrors the
                    // touchKeyLastUsed pattern in /api/internal-validate-api-key.
                    console.warn("[validate-pro-mcp-token] touch schedule failed:", err instanceof Error ? err.message : String(err));
                }
            }
            return new Response(JSON.stringify(result), {
                status: 200,
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        try {
            const result = await ctx.runMutation(internal.mcpProTokens.internalRevokeProMcpToken, { userId: body.userId, tokenId: body.tokenId });
            return new Response(JSON.stringify(result), {
                status: 200,
                headers: { "Content-Type": "application/json" },
            });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        if (!body.userId || !body.productId) {
            return new Response(JSON.stringify({ error: "MISSING_FIELDS", required: ["userId", "productId"] }), { status: 400, headers: { "Content-Type": "application/json" } });
        }
        try {
            const result = await ctx.runAction(internal.payments.checkout.internalCreateCheckout, {
                userId: body.userId,
                email: body.email,
                name: body.name,
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        if (!body.userId) {
            return new Response(JSON.stringify({ error: "MISSING_FIELDS", required: ["userId"] }), { status: 400, headers: { "Content-Type": "application/json" } });
        }
        try {
            const result = await ctx.runAction(internal.payments.billing.internalGetCustomerPortalUrl, { userId: body.userId });
            return new Response(JSON.stringify(result), {
                status: 200,
                headers: { "Content-Type": "application/json" },
            });
...
http.route({
    ...
    handler: httpAction(async (ctx, request) => {
        ...
        if (!Array.isArray(body.emails) || body.emails.length === 0) {
            return new Response(JSON.stringify({ error: "MISSING_FIELDS", required: ["emails"] }), { status: 400, headers: { "Content-Type": "application/json" } });
        }
        try {
            const result = await ctx.runMutation(internal.emailSuppressions.bulkSuppress, { emails: body.emails });
            return new Response(JSON.stringify(result), {
                status: 200,
                headers: { "Content-Type": "application/json" },
            });

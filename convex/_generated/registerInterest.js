        ...
        if (args.referredBy) {
            ...
            if (registrationReferrer) {
                ...
                const clerkReferrer = await ctx.db
                    ...
                    .withIndex("by_code", (q) => q.eq("code", args.referredBy))
                    .first();
                if (clerkReferrer) {
                    ...
                    const existingCredit = await ctx.db
                        ...
                        .withIndex("by_referrer_email", (q) => q.eq("referrerUserId", clerkReferrer.userId).eq("refereeEmail", normalizedEmail))
                        .first();
                    if (!existingCredit) {
    ...
    handler: async (ctx, args) => {
        ...
        if (existing) {
            if (existing.userId !== args.userId) {
                console.warn(`[referral] code collision: ${args.code} first bound to ${existing.userId}, ignoring request from ${args.userId}`);
            }
            return { isNew: false };
        }

...
export function getEntitlementFeatures(planKey) {
    const entry = PRODUCT_CATALOG[planKey];
    if (!entry) {
        throw new Error(`[productCatalog] Unknown planKey "${planKey}". Add it to PRODUCT_CATALOG.`);
    }
    return entry.features;
}
export function resolveProductToPlan(dodoProductId) {
    const entry = Object.values(PRODUCT_CATALOG).find((e) => e.dodoProductId === dodoProductId);
    if (entry)
...
export function getSeedableProducts() {

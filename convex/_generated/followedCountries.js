...
import { COUNTRY_COUNT_PRIVACY_FLOOR, FREE_TIER_FOLLOW_LIMIT, MAX_MERGE_INPUT, SHARD_COUNT, } from "./constants";
 ...
 */
async function readEntitlementTier(ctx, userId) {
 ...
 */
async function readShardOrThrow(ctx, userId) {
 ...
 */
async function touchShard(ctx, shard) {
    await ctx.db.patch(shard._id, { lastTouchedAt: Date.now() });
}
async function readCountryLockOrThrow(ctx, country) {
...
async function touchCountryLock(ctx, lock) {
 ...
 */
async function readUserMeta(ctx, userId) {
 ...
 */
async function writeUserMeta(ctx, userId, meta, newCount) {
    const now = Date.now();
    if (meta) {
...
async function updateCountryCounter(ctx, country, delta) {
 ...
 */
async function incrementCountryCounter(ctx, country) {
 ...
 */
async function decrementCountryCounter(ctx, country) {
    await updateCountryCounter(ctx, country, -1);
}
async function readRawCountryFollowerCount(ctx, country) {
    const rows = await ctx.db
        .query("followedCountriesCounts")
        .withIndex("by_country", (q) => q.eq("country", country))
        .collect();
...
export const followCountry = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!identity)
            throw new ConvexError({ kind: "UNAUTHENTICATED" });
        ...
        const existingRow = await ctx.db
            ...
            .withIndex("by_user_country", (q) => q.eq("userId", userId).eq("country", args.country))
            .first();
        if (existingRow) {
...
export const mergeAnonymousLocal = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingRows = await ctx.db
            ...
            .withIndex("by_user", (q) => q.eq("userId", userId))
            .collect();
        const existingSet = new Set(existingRows.map((r) => r.country));
...
export const countFollowers = query({
    ...
    handler: async (ctx, args) => {
        ...
        // is returned exactly.
        if (raw < COUNTRY_COUNT_PRIVACY_FLOOR)
            return 0;
        return raw;
    },
});
...
export const _seedShards = internalMutation({
    ...
    handler: async (ctx) => {
        const existing = await ctx.db
            .query("followedCountriesShards")
            .collect();
        const have = new Set(existing.map((r) => r.shardId));
        const now = Date.now();
        let seeded = 0;
...
export const _seedCountryLocks = internalMutation({
    ...
    handler: async (ctx) => {
        const existing = await ctx.db
            .query("followedCountriesCountryLocks")
            .collect();
        const have = new Set(existing.map((r) => r.country));
        const now = Date.now();
        let seeded = 0;

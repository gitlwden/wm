...
function getSigningKey() {
    ...
    if (!key) {
        throw new Error("[identity-signing] DODO_IDENTITY_SIGNING_SECRET not set. " +
 ...
 */
export async function signUserId(userId) {
    const key = getSigningKey();
    const encoder = new TextEncoder();
    const cryptoKey = await crypto.subtle.importKey("raw", encoder.encode(key), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    const signature = await crypto.subtle.sign("HMAC", cryptoKey, encoder.encode(userId));
 ...
 */
export async function verifyUserId(userId, signature) {
    try {

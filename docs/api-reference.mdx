...
```
https://api.wm.vercel.app/api/<service>/v1/<rpc-name>
```
...
Building an agent or codegen pipeline? Point at `https://wm.vercel.app/` and follow the `Link:` header — every OpenAPI spec, MCP server card, OAuth endpoint, and agent-skill bundle is discoverable from one root URL via standard `.well-known` paths. See [Agent Discovery](/agent-discovery) for the full walkthrough.

The single bundled spec is at [`/openapi.yaml`](https://wm.vercel.app/openapi.yaml) — feed it to any OpenAPI generator to produce typed clients for all 34 services in one pass.
...
Always cross-check against [the bundled OpenAPI spec](https://wm.vercel.app/openapi.yaml) — that's the source of truth for path names, parameters, and response shapes.

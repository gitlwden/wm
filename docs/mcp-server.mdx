...
| Endpoint | Purpose |
|----------|---------|
| `https://wm.vercel.app/mcp` | JSON-RPC server (Streamable HTTP transport, protocol `2025-03-26`) |
| `https://api.wm.vercel.app/api/oauth/register` | Dynamic Client Registration (RFC 7591) |
| `https://api.wm.vercel.app/api/oauth/authorize` | OAuth 2.1 authorization endpoint (PKCE required) |
| `https://api.wm.vercel.app/api/oauth/token` | Token endpoint (authorization_code + refresh_token) |
| `https://api.wm.vercel.app/.well-known/oauth-authorization-server` | AS metadata (RFC 8414) |
| `https://wm.vercel.app/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |
...
1. **Add the server URL** in your MCP client. The canonical entrypoint is:
   ...
   ```
   https://api.wm.vercel.app/mcp
   ```

   (`https://wm.vercel.app/mcp` works too — it proxies the same handler.)

2. **Click "Sign in with WorldMonitor Pro"** on the consent page. This is the default CTA. You'll bounce through `wm.vercel.app/mcp-grant` (Clerk-protected — sign in if needed), then back to `api.wm.vercel.app/oauth/authorize-pro`, then to your client's redirect.
...
- Manage connected clients in **[Settings → Connected MCP clients](https://wm.vercel.app/settings)**.
...
|------|------------|------------|-------------|-------|
| **Free** | ❌ No | — | — | OAuth flow returns 401 `INSUFFICIENT_TIER`. Upgrade at [wm.vercel.app/pro](https://wm.vercel.app/pro). |
...
```bash
npx @modelcontextprotocol/inspector https://wm.vercel.app/mcp
```
...
# 1. List tools
curl -s https://wm.vercel.app/mcp \
...
Seed-level health per key: [status.wm.vercel.app](https://status.wm.vercel.app/).
...
- [Pro features](https://wm.vercel.app/pro)

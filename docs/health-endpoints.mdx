...
```bash
# Full health check
curl -s https://api.wm.vercel.app/api/health | jq .

# Compact (problems only)
curl -s "https://api.wm.vercel.app/api/health?compact=1" | jq .

# UptimeRobot / monitoring: check HTTP status code
curl -o /dev/null -s -w "%{http_code}" https://api.wm.vercel.app/api/health
...
```bash
curl -s https://api.wm.vercel.app/api/seed-health \
  -H "Origin: https://wm.vercel.app" | jq .
```
...
Point the keyword monitor at `https://api.wm.vercel.app/api/health?compact=1` and alert when the compact token `"status":"HEALTHY"` (no space after the colon) is **absent** from the response body. Compact mode serializes with no indentation, so this exact token is stable regardless of formatting.
...
STATUS=$(curl -s "https://api.wm.vercel.app/api/health?compact=1")

...
Products are Dodo `productId`s stored client-side in `pro-test/src/generated/tiers.json` and served at runtime from `https://api.wm.vercel.app/api/product-catalog`:
...
- **Enterprise** — `mailto:enterprise@wm.vercel.app` (contact sales).
...
1. Validates `returnUrl` against an allow-listed set of wm.vercel.app origins.
...
- **Full-page return** (fallback / `/pro` path): Dodo redirects to `wm.vercel.app/?subscription_id=...&status=active`. `src/services/checkout-return.ts:handleCheckoutReturn()` reads params, cleans the URL, returns success boolean.
...
- **Share link**: `https://wm.vercel.app/pro?ref=<code>`.

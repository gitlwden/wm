...
description: "Point any AI agent, codegen tool, or MCP client at wm.vercel.app and discover every API, OpenAPI spec, MCP server, and OAuth endpoint from a single root URL."
---
...
No prior knowledge required. Just `GET https://wm.vercel.app/`.
...
```bash
curl -sI https://wm.vercel.app/ | grep -i '^link:'
```
...
|---|---|---|
| [`/.well-known/api-catalog`](https://wm.vercel.app/.well-known/api-catalog) | [RFC 9727](https://datatracker.ietf.org/doc/html/rfc9727) | JSON linkset bundling every other discovery URL — start here if you want one fetch and a map |
| [`/openapi.yaml`](https://wm.vercel.app/openapi.yaml) | OpenAPI 3.1 | Single bundled spec covering **every** REST service (Conflict, Resilience, Market, Economic, Maritime, Aviation, Climate, …) — feed it to any code-generator |
| [`/.well-known/mcp/server-card.json`](https://wm.vercel.app/.well-known/mcp/server-card.json) | MCP | Transport (`streamableHttp`), endpoint, OAuth resource, scopes, capability flags |
| [`/.well-known/oauth-authorization-server`](https://api.wm.vercel.app/.well-known/oauth-authorization-server) | [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) | OAuth 2.1 authorization-server metadata (PKCE, DCR, token endpoints) |
| [`/.well-known/oauth-protected-resource`](https://wm.vercel.app/.well-known/oauth-protected-resource) | [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) | Resource metadata for `https://wm.vercel.app/mcp` |
| [`/.well-known/agent-skills/index.json`](https://wm.vercel.app/.well-known/agent-skills/index.json) | (custom) | Index of pre-packaged agent skills (`fetch-country-brief`, `fetch-resilience-score`, …) — each skill is a self-contained recipe |
| [`/llms.txt`](https://wm.vercel.app/llms.txt) | [llmstxt.org](https://llmstxt.org) | LLM-friendly markdown briefing — overview, capabilities, links |
| [`/llms-full.txt`](https://wm.vercel.app/llms-full.txt) | (extension) | Long-form variant covering all data layers, panels, and data sources |
| [`/api/health`](https://api.wm.vercel.app/api/health) | (custom) | Per-key seed status, freshness, record counts — agents can gate fetches on this |
...
curl -s https://wm.vercel.app/.well-known/api-catalog \
  | jq -r '.linkset[0]."service-desc"[0].href'
# → https://wm.vercel.app/openapi.yaml

# 2. Generate clients
curl -s https://wm.vercel.app/openapi.yaml -o worldmonitor.openapi.yaml
...
curl -s https://wm.vercel.app/.well-known/mcp/server-card.json
# → endpoint (https://wm.vercel.app/mcp), transport, OAuth scopes,
#   and authorization_servers: ["https://api.wm.vercel.app"]

# 2. Fetch authorization-server metadata from that host
curl -s https://api.wm.vercel.app/.well-known/oauth-authorization-server
...
```bash
curl -s https://api.wm.vercel.app/api/resilience/v1/get-resilience-ranking \
  -H "X-WorldMonitor-Key: $WM_KEY"
```

PRO subscribers get a key from [wm.vercel.app/pro](https://wm.vercel.app/pro). See [Authentication](/usage-auth).

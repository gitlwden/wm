...
Upgrade or generate a key at [wm.vercel.app/pro](https://wm.vercel.app/pro). The rest of this guide assumes Claude Desktop + OAuth (the simplest path); if you'd rather paste a `wm_…` key into a `curl` script, jump to [Server-side curl](#server-side-curl) at the bottom.
...
```bash
export WM_KEY="wm_live_..."  # API Starter+ key from wm.vercel.app/settings

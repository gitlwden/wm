...
const SOURCE_METADATA: Record<DataSourceId, { name: string; requiredForRisk: boolean; panelId?: string }> = {
  ...
  acled_conflict: { name: 'Armed Conflicts (GDELT)', requiredForRisk: false, panelId: 'ucdp-events' },
...
const INTELLIGENCE_GAP_MESSAGES: Record<DataSourceId, string> = {
  ...
  acled_conflict: 'Armed conflict events may be missed—GDELT conflict data unavailable',
  ...
  ucdp_events: 'Armed conflict event data unavailable',

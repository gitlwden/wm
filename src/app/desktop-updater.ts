...
export class DesktopUpdater implements AppModule {
  ...
  private async checkForUpdate(): Promise<void> {
    try {
      const res = await fetch('https://wm-worldmonitor.netlify.app/api/version', {
  ...
  private async resolveUpdateDownloadUrl(releaseUrl: string): Promise<string> {
    try {
      ...
      if (platform) {
        ...
        return `https://wm-worldmonitor.netlify.app/api/download?platform=${platform}&variant=${variant}`;
      }
    } catch {

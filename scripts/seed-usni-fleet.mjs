...
function envelopeWrite(key, data, ttlSeconds, meta) {
  ...
  const envelope = buildEnvelope({
    fetchedAt: Date.now(),
    recordCount,
    sourceVersion: meta?.sourceVersion || 'usni-fleet',
...
async function main() {
  ...
  if (!res.ok) {
    console.warn(`[USNI] Fetch failed: HTTP ${res.status}`);
    process.exit(0);
  }

  const wpData = await res.json();
  if (!Array.isArray(wpData) || !wpData.length) {
    console.warn('[USNI] No fleet tracker articles');
    process.exit(0);
  }
  ...
  if (!htmlContent) {
    console.warn('[USNI] Empty article content');
    process.exit(0);
  }
...
main().catch((e) => {
  console.error('[USNI] Fatal error:', e?.message || e);
  process.exit(1);
});

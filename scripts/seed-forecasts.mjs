...
async function warmPingChokepoints() {
  ...
  try {
    const resp = await fetch(`${baseUrl}/api/supply-chain/v1/get-chokepoint-status`, {
      headers: { 'User-Agent': CHROME_UA, Origin: 'https://wm-worldmonitor.netlify.app' },
    ...
    try {
      const resp = await fetch(provider.apiUrl, {
        ...
        headers: {
          ...
          ...(provider.name === 'openrouter' ? { 'HTTP-Referer': 'https://wm-worldmonitor.netlify.app', 'X-Title': 'World Monitor' } : {}),
        },
        body: JSON.stringify({
